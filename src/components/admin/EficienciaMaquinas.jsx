...
export default function EficienciaMaquinas() {
  ...
  const { data: rawData = [], isLoading } = useQuery({
    queryKey: ["admin-eficiencia-v2", weekAgoStr],
    queryFn: async () => {
      ...
      const stats = {};
      const add = (machine, prod, setup, pausa, kg, setor) => {
        if (!machine) return;
        if (!stats[machine]) stats[machine] = { producao: 0, setup: 0, pausa: 0, kg: 0, setor };
        stats[machine].producao += prod || 0;
        stats[machine].setup += setup || 0;
        stats[machine].pausa += pausa || 0;
        stats[machine].kg += kg || 0;
      };
  ...
  const filteredData = useMemo(() => {
    ...
    if (buscaMaquina.trim()) {
      const q = buscaMaquina.toLowerCase().trim();
      d = d.filter(m => m.machine.toLowerCase().includes(q));
    }
    return d;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" value={buscaMaquina} onChange={e => setBuscaMaquina(e.target.value)} placeholder="Buscar máquina..."
      ...
      {filteredData.length > 0 ? (
        <ResponsiveContainer width="100%" height={350}>
          <BarChart data={filteredData}>
            ...
            <XAxis dataKey="machine" tick={{ fontSize: 10 }} angle={-20} textAnchor="end" height={60} interval={0} />
            <YAxis tick={{ fontSize: 11 }} />
            <Tooltip formatter={(v, n) => n.includes("KG") ? `${v} kg` : `${v}h`} />
            <Legend />
        ...
        <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground">Nenhum dado de produção na semana.</div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border text-xs text-muted-foreground bg-muted/30">
              ...
              <th className="text-right py-2 px-4 font-medium">Horas Produção</th>
              <th className="text-right py-2 px-4 font-medium">Horas Setup</th>
              <th className="text-right py-2 px-4 font-medium">Horas Pausa</th>
              <th className="text-right py-2 px-4 font-medium">KG Produzido</th>
              <th className="text-right py-2 px-4 font-medium">Eficiência</th>
            </tr></thead>
            <tbody>
              {filteredData.map(m => (
                <tr key={m.machine} className="border-b border-border hover:bg-muted/30">
                  ...
                  <td className="py-2.5 px-4 text-right text-emerald-600 font-medium">{m.producao}h</td>
                  <td className="py-2.5 px-4 text-right text-amber-600">{m.setup}h</td>
                  <td className="py-2.5 px-4 text-right text-red-500">{m.pausa}h</td>
                  <td className="py-2.5 px-4 text-right font-bold">{m.kg.toLocaleString('pt-BR')} kg</td>
                  <td className="py-2.5 px-4 text-right">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${m.eficiencia >= 70 ? "bg-emerald-100 text-emerald-700" : m.eficiencia >= 50 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>{m.eficiencia}%</span>
                  </td>
                </tr>
              ))}
            </tbody>
