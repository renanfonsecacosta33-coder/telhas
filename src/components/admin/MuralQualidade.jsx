...
function getTimeline(ops) {
  ...
  const stageStatus = (list) => {
    if (list.length === 0) return "none";
    if (list.every(o => o.status === "finalizado")) return "done";
    if (list.some(o => o.status === "em_producao" || o.status === "aguardando_corte")) return "producing";
    return "pending";
  };

  return {
    corte: stageStatus(corteOps),
    dobra: stageStatus(dobraOps),
    logistica: anyExpedicao ? (anyExpedicao.status_expedicao === "expedido" ? "done" : "producing") : (allFinalizado ? "pending" : "none"),
  };
}

const STAGE_CFG = {
  done: { color: "bg-emerald-500 text-white", label: "Finalizado" },
  producing: { color: "bg-blue-500 text-white", label: "Produzindo" },
  pending: { color: "bg-amber-400 text-white", label: "Pendente" },
  none: { color: "bg-muted text-muted-foreground", label: "—" },
};
...
export default function MuralQualidade() {
  ...
  const { data: grupos = [], isLoading } = useQuery({
    queryKey: ["admin-mural-v2"],
    queryFn: async () => {
      const [ordensMaq, ordensDesb, pedidos] = await Promise.all([
        base44.entities.OrdemMaquinaCD.list("-created_date", 100),
        base44.entities.OrdemDesbobinadeira.list("-created_date", 100),
      ...
      Object.values(opsByPedido).forEach(grupo => {
        ...
        grupo.ops.forEach(op => {
          if (op.foto_finalizacao_url) fotos.push({ url: op.foto_finalizacao_url, tipo: "Finalização", data: op.data_finalizacao, maquina: op.maquina || op._tipo });
          if (op.foto_carregamento_url) fotos.push({ url: op.foto_carregamento_url, tipo: "Carregamento", data: op.data_finalizacao, maquina: op.maquina || op._tipo });
        });
        if (fotos.length > 0) {
      ...
      return grupos.sort((a, b) => {
        const aDate = a.fotos[0]?.data || "";
        const bDate = b.fotos[0]?.data || "";
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {grupo.fotos.map((f, j) => (
                    <div key={j} className="flex-shrink-0 relative group">
                      ...
                      <button onClick={() => setZoomPhoto(f.url)} className="absolute top-1 right-1 bg-black/60 text-white rounded p-1 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {zoomPhoto && (
        <Dialog open={!!zoomPhoto} onOpenChange={() => setZoomPhoto(null)}>
          <DialogContent className="sm:max-w-4xl p-2">
            <img src={zoomPhoto} alt="Foto ampliada" className="w-full h-auto rounded-lg" />
          </DialogContent>
        </Dialog>
      )}
    </div>
