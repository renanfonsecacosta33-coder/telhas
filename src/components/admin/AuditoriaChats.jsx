...
import { Search, Factory, ClipboardList, MessageCircle, Plus, User, FileText } from "lucide-react";
...
const MAQUINAS = [
  { nome: "TP - 25", setor: "Telhas" }, { nome: "TP - 40", setor: "Telhas" }, { nome: "ONDULADA", setor: "Telhas" }, { nome: "COLONIAL", setor: "Telhas" }, { nome: "BANDEJA", setor: "Telhas" }, { nome: "DESBOBINADOR", setor: "Telhas" }, { nome: "CUMEEIRA", setor: "Telhas" }, { nome: "COLAGEM", setor: "Telhas" },
  { nome: "DESBobINADEIRA", setor: "Corte e Dobra" }, { nome: "CORTE 3M", setor: "Corte e Dobra" }, { nome: "DOBRA 3M", setor: "Corte e Dobra" }, { nome: "CORTE 6M", setor: "Corte e Dobra" }, { nome: "DOBRA FUNDO 6M", setor: "Corte e Dobra" }, { nome: "DOBRA INICIO 6M", setor: "Corte e Dobra" }, { nome: "PERFILADEIRA", setor: "Corte e Dobra" },
];
...
export default function AuditoriaChats() {
  ...
  const { data: mensagens = [], isLoading } = useQuery({
    queryKey: ["admin-auditoria-chats-v3"],
  ...
  const { data: pedidosRecentes = [] } = useQuery({
    ...
    queryFn: async () => {
      const [p, om, od] = await Promise.all([
        base44.entities.Pedido.list("-created_date", 50), base44.entities.OrdemMaquinaCD.list("-created_date", 50), base44.entities.OrdemDesbobinadeira.list("-created_date", 50),
      ]);
      return [...p.map(x => ({ id: x.id, label: `#${x.numero_pedido || "—"} ${x.cliente || ""}` })), ...om.map(x => ({ id: x.id, label: `OP CD ${x.numero_pedido || "—"} ${x.cliente || ""}` })), ...od.map(x => ({ id: x.id, label: `OP DESB ${x.numero_pedido || "—"} ${x.cliente || ""}` }))];
    },
  });
  ...
  const canais = useMemo(() => {
    ...
    mensagens.forEach(m => {
      ...
      map[key].total++; if (new Date(m.data_hora) > new Date(map[key].ultima_msg.data_hora)) map[key].ultima_msg = m;
    });
  ...
  const equipeFiltrada = useMemo(() => { if (!buscaModal.trim()) return equipe; const q = buscaModal.toLowerCase(); return equipe.filter(f => f.full_name?.toLowerCase().includes(q)); }, [equipe, buscaModal]);
  const maquinasFiltradas = useMemo(() => { if (!buscaModal.trim()) return MAQUINAS; const q = buscaModal.toLowerCase(); return MAQUINAS.filter(m => m.nome.toLowerCase().includes(q)); }, [buscaModal]);
  const pedidosFiltrados = useMemo(() => { if (!buscaModal.trim()) return pedidosRecentes; const q = buscaModal.toLowerCase(); return pedidosRecentes.filter(p => p.label.toLowerCase().includes(q)); }, [pedidosRecentes, buscaModal]);
  ...
  const iniciar = (canal) => { setCanalSelecionado(canal); setShowIniciar(false); setBuscaModal(""); };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <div className="relative flex-1"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" /><Input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar canal..." className="h-9 pl-8 pr-3" /></div>
        ...
        {canalSelecionado ? (
          <div className="flex-1 flex flex-col border border-border rounded-xl overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden">
              {user && <ChatPanel canal_tipo={canalSelecionado.tipo} canal_id={canalSelecionado.id} canal_label={canalSelecionado.label} currentUser={user} destinatarioId={canalSelecionado.destinatarioId} heightClass="h-[calc(100vh-200px)]" />}
            </div>
          </div>
        ) : <div className="hidden sm:flex flex-1 items-center justify-center border border-border rounded-xl text-sm text-muted-foreground">Selecione um canal ou inicie uma nova conversa</div>}
      </div>

      {/* Iniciar Conversa Modal com busca */}
      <Dialog open={showIniciar} onOpenChange={(o) => { setShowIniciar(o); if (!o) setBuscaModal(""); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Iniciar Conversa</DialogTitle></DialogHeader>
          <div className="relative mb-3">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
            <Input value={buscaModal} onChange={(e) => setBuscaModal(e.target.value)} placeholder="Buscar por nome, máquina ou pedido..." className="h-9 pl-8 pr-3" autoFocus />
          </div>
          <Tabs defaultValue="funcionario">
            ...
            <TabsContent value="funcionario">
              <div className="max-h-[300px] overflow-y-auto space-y-1">
                {equipeFiltrada.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">Nenhum resultado.</p> : equipeFiltrada.map(f => (
                  <button key={f.id} onClick={() => iniciar({ tipo: "direto", id: getDirectChannelId(user?.id, f.id), label: f.full_name, destinatarioId: f.id })} className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-muted transition-colors text-left">
            ...
            <TabsContent value="maquina">
              <div className="max-h-[300px] overflow-y-auto space-y-1">
                {maquinasFiltradas.map(m => (
                  <button key={m.nome} onClick={() => iniciar({ tipo: "maquina", id: m.nome, label: m.nome })} className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-muted transition-colors text-left">
            ...
            <TabsContent value="pedido">
              <div className="max-h-[300px] overflow-y-auto space-y-1">
                {pedidosFiltrados.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">Nenhum resultado.</p> : pedidosFiltrados.map(p => (
                  <button key={p.id} onClick={() => iniciar({ tipo: "pedido", id: p.id, label: p.label })} className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-muted transition-colors text-left">
